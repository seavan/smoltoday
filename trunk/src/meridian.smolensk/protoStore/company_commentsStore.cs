...
{
	public partial class company_commentsStore : Meridian.IEntityStore, admin.db.IDataService<proto.company_comments>
	{
		public company_commentsStore()
		{
			m_Items = new SortedList<long, proto.company_comments>();
		}
		private SortedList<long, proto.company_comments> m_Items;
		public void Commit()
		{
			//throw new NotImplementedException();
		}
		public IQueryable<proto.company_comments> GetAll()
		{
			return Queryable.AsQueryable<proto.company_comments>(All());
		}
		public proto.company_comments GetById(long id)
		{
			return Get(id);
		}
		public proto.company_comments CreateItem()
		{
			return new proto.company_comments() {   };
		}
		public void AbortItem(proto.company_comments item)
		{
			Delete(item);
		}
		public void Discard()
		...
		{
			return typeof(proto.company_comments);
		}
		...
		{
			var cmd = new MySqlCommand("SELECT `id`, `left_key`, `right_key`, `level`, `delete`, `create_date`, `account_id`, `text`, `parent_id`, `company_id` FROM company_comments");
			cmd.Connection = _connection;
			using(var reader = cmd.ExecuteReader())
			{
				while(reader.Read())
				{
					var item = new proto.company_comments();
					item.LoadFromReader(reader);
					m_Items[item.id] = item;
				}
			}
		}
		public proto.company_comments Insert(MySqlConnection _connection, proto.company_comments _item)
		{
			var cmd = new MySqlCommand("INSERT INTO company_comments ( `left_key`, `right_key`, `level`, `delete`, `create_date`, `account_id`, `text`, `parent_id`, `company_id` ) VALUES ( @left_key, @right_key, @level, @delete, @create_date, @account_id, @text, @parent_id, @company_id ); SELECT LAST_INSERT_ID();"); ;
		...
		public proto.company_comments Update(MySqlConnection _connection, proto.company_comments _item)
		{
			bool changed =  false;
			var cmdText =  "";
			var cmd = new MySqlCommand("UPDATE company_comments SET `left_key`= @left_key, `right_key`= @right_key, `level`= @level, `delete`= @delete, `create_date`= @create_date, `account_id`= @account_id, `text`= @text, `parent_id`= @parent_id, `company_id`= @company_id WHERE id=@id"); ;
			if(_item.mc_id)
			{
			}
			...
			if(changed)
			{
				cmd.CommandText =  "UPDATE company_comments SET " + cmdText + " WHERE id=@id";
				cmd.Connection = _connection;
				cmd.ExecuteNonQuery();
				_item.LoadAggregations(Meridian.Default);
			}
		...
		public proto.company_comments Delete(MySqlConnection _connection, proto.company_comments _item)
		{
			var cmd = new MySqlCommand("DELETE FROM company_comments WHERE id=@id"); ;
			cmd.Parameters.Add( new MySqlParameter() { ParameterName = "id", Value = _item.id });
			cmd.Connection = _connection;
			cmd.ExecuteScalar();
			return _item;
		}
		public proto.company_comments Insert(proto.company_comments _item)
		{
			MeridianMonitor.Default.MySqlActionForeground((conn) => Insert(conn, _item));;
			return _item;
		}
		public proto.company_comments Persist(proto.company_comments _item)
		{
			if(_item.id <= 0)
			{
				_item = Insert(_item);
		...
		public proto.company_comments Delete(proto.company_comments _item)
		{
		...
		public proto.company_comments Update(proto.company_comments _item)
		{
			MeridianMonitor.Default.MySqlActionBackground((conn) => Update(conn, _item));;
			return _item;
		}
		public IList<proto.company_comments> All()
		{
			return m_Items.Values;
		}
		public proto.company_comments Get(long _id)
		{
			return m_Items[_id];
		}
		...
		public void UpdateSync(MySqlConnection _connection, long _id, Meridian _meridian)
		{
			...
			var item = Get(_id);
			var cmd = new MySqlCommand("SELECT `id`, `left_key`, `right_key`, `level`, `delete`, `create_date`, `account_id`, `text`, `parent_id`, `company_id` FROM company_comments WHERE id = " + _id.ToString());
			cmd.Connection = _connection;
			using(var reader = cmd.ExecuteReader())
			{
				while(reader.Read())
