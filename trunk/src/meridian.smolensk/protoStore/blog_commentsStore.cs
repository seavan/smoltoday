...
{
	public partial class blog_commentsStore : Meridian.IEntityStore, admin.db.IDataService<proto.blog_comments>
	{
		public blog_commentsStore()
		{
			m_Items = new SortedList<long, proto.blog_comments>();
		}
		private SortedList<long, proto.blog_comments> m_Items;
		public void Commit()
		{
			//throw new NotImplementedException();
		}
		public IQueryable<proto.blog_comments> GetAll()
		{
			return Queryable.AsQueryable<proto.blog_comments>(All());
		}
		public proto.blog_comments GetById(long id)
		{
			return Get(id);
		}
		void admin.db.IDataService<proto.blog_comments>.Insert(proto.blog_comments item)
		{
			Insert(item);
		}
		void admin.db.IDataService<proto.blog_comments>.Delete(proto.blog_comments item)
		{
			Delete(item);
		}
		public proto.blog_comments CreateItem()
		{
			return new proto.blog_comments() {   };
		}
		public void AbortItem(proto.blog_comments item)
		{
			Delete(item);
		}
		public void Discard()
		...
		void admin.db.IDataService<proto.blog_comments>.Update(proto.blog_comments item)
		{
			Persist(item);
		}
		...
		{
			return typeof(proto.blog_comments);
		}
		...
		{
			var cmd = new MySqlCommand("SELECT `id`, `left_key`, `right_key`, `level`, `delete`, `create_date`, `account_id`, `text`, `parent_id`, `blog_id` FROM blog_comments");
			cmd.Connection = _connection;
			using(var reader = cmd.ExecuteReader())
			{
				while(reader.Read())
				{
					var item = new proto.blog_comments();
					item.LoadFromReader(reader);
					m_Items[item.id] = item;
				}
			}
		}
		public proto.blog_comments Insert(MySqlConnection _connection, proto.blog_comments _item)
		{
			var cmd = new MySqlCommand("INSERT INTO blog_comments ( `left_key`, `right_key`, `level`, `delete`, `create_date`, `account_id`, `text`, `parent_id`, `blog_id` ) VALUES ( @left_key, @right_key, @level, @delete, @create_date, @account_id, @text, @parent_id, @blog_id ); SELECT LAST_INSERT_ID();"); ;
		...
		public proto.blog_comments Update(MySqlConnection _connection, proto.blog_comments _item)
		{
			bool changed =  false;
			var cmdText =  "";
			var cmd = new MySqlCommand("UPDATE blog_comments SET `left_key`= @left_key, `right_key`= @right_key, `level`= @level, `delete`= @delete, `create_date`= @create_date, `account_id`= @account_id, `text`= @text, `parent_id`= @parent_id, `blog_id`= @blog_id WHERE id=@id"); ;
			if(_item.mc_id)
			{
			}
			...
			if(changed)
			{
				cmd.CommandText =  "UPDATE blog_comments SET " + cmdText + " WHERE id=@id";
				cmd.Connection = _connection;
				cmd.ExecuteNonQuery();
				_item.LoadAggregations(Meridian.Default);
			}
		...
		public proto.blog_comments Delete(MySqlConnection _connection, proto.blog_comments _item)
		{
			var cmd = new MySqlCommand("DELETE FROM blog_comments WHERE id=@id"); ;
			cmd.Parameters.Add( new MySqlParameter() { ParameterName = "id", Value = _item.id });
			cmd.Connection = _connection;
			cmd.ExecuteScalar();
			return _item;
		}
		public proto.blog_comments Insert(proto.blog_comments _item)
		{
			MeridianMonitor.Default.MySqlActionForeground((conn) => Insert(conn, _item));;
			return _item;
		}
		public proto.blog_comments Persist(proto.blog_comments _item)
		{
			if(_item.id <= 0)
			{
				_item = Insert(_item);
		...
		public proto.blog_comments Delete(proto.blog_comments _item)
		{
		...
		public proto.blog_comments Update(proto.blog_comments _item)
		{
			MeridianMonitor.Default.MySqlActionBackground((conn) => Update(conn, _item));;
			return _item;
		}
		public IList<proto.blog_comments> All()
		{
			return m_Items.Values;
		}
		public proto.blog_comments Get(long _id)
		{
			return m_Items[_id];
		}
		...
		public void UpdateSync(MySqlConnection _connection, long _id, Meridian _meridian)
		{
			...
			var item = Get(_id);
			var cmd = new MySqlCommand("SELECT `id`, `left_key`, `right_key`, `level`, `delete`, `create_date`, `account_id`, `text`, `parent_id`, `blog_id` FROM blog_comments WHERE id = " + _id.ToString());
			cmd.Connection = _connection;
			using(var reader = cmd.ExecuteReader())
			{
				while(reader.Read())
