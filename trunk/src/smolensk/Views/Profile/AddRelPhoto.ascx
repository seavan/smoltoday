...
<div class="grayCreateBlock form_addRelPhoto" style="display:none;">
    
    <form action="#" class="ajaxForm" data-action="<%: Url.Action("AddRelPhoto", "Profile")%>" data-target=".form_addRelPhoto" data-callback="hideOverlay">
        <%: Html.HiddenFor(m=>m.AlbumId) %> 
        <%: Html.HiddenFor(m=>m.ParentId) %> 
    <table class="createBlock">   
        <tr>
            <th colspan="2">Добавление варианта фото</th>
        </tr>                 	
        <tr>
            <td>Название</td>
            <td><%= Model.Title %></td>
